; cpm22_pc98.asm
;
; Intel 8080 CP/M 2.2 CCP+BDOS core for the PC-9801 V30 8080-mode port.
; HDD-enabled 62K layout: CCP=DC00h, BDOS=E406h, BIOS=F200h.
; Expected binary size: 1600h bytes (CCP+BDOS only).
; CP/M 2.2 CCP+BDOS core adapted to the 62K PC-9801 layout used in this archive.
;
...
;
MEM     EQU     62      ;62K CP/M-80; F800h-FFFFh reserved for two HDD ALVs
;
...
RESBOOT         EQU     0F7E0H  ;CBIOS private: BDOS reset on physical boot drive
BOOTSEL         EQU     0F7E9H  ;CBIOS private: BDOS select physical boot drive
;
...
DEL     EQU     7FH     ;rubout
;
;   Standard CP/M 2.2 placement for MEM=62.
;
BIAS    EQU     (MEM-20)*1024
CCPBASE         EQU     03400H+BIAS
BDOSBASE EQU    CCPBASE+0806H
BIOSBASE EQU    CCPBASE+1600H
;
; Bind all BDOS BIOS calls directly to the separately assembled CBIOS.
BOOT    EQU     BIOSBASE+0
WBOOT   EQU     BIOSBASE+3
CONST   EQU     BIOSBASE+6
CONIN   EQU     BIOSBASE+9
CONOUT  EQU     BIOSBASE+12
LIST    EQU     BIOSBASE+15
PUNCH   EQU     BIOSBASE+18
READER  EQU     BIOSBASE+21
HOME    EQU     BIOSBASE+24
SELDSK  EQU     BIOSBASE+27
SETTRK  EQU     BIOSBASE+30
SETSEC  EQU     BIOSBASE+33
SETDMA  EQU     BIOSBASE+36
READ    EQU     BIOSBASE+39
WRITE   EQU     BIOSBASE+42
PRSTAT  EQU     BIOSBASE+45
SECTRN  EQU     BIOSBASE+48
;
        ORG     CCPBASE
;
...
DELBATCH:LXI    H,BATCH         ;is batch active?
        ...
        CALL    BOOTSEL         ;select physical boot drive
        NOP             ;preserve original 4-byte footprint
...
COMMAND         LXI     SP,CCPSTACK;setup stack area.
        ...
        CALL    RESBOOT         ;reset disk system on physical boot drive.
...
;**************************************************************
;
; Pad reconstructed CCP to the standard BDOS entry address.
        DS      BDOSBASE-$
FBASE   JMP     FBASE1
;
;   Bdos error table.
...
RSTDSK  LXI     H,0     ;clear write protect status and log
        ...
        SHLD    LOGIN
        MOV     A,C     ;function 13 private parameter: physical boot drive
        STA     ACTIVE
...
;**************************************************************
;*
;*        E N D   O F   C C P + B D O S
;*
;**************************************************************
;
; Keep the separately assembled CBIOS at FA00h.  No dummy BIOS jump table
; bytes are emitted in this core image.
        DS      BIOSBASE-$
;
;*
;******************   E N D   O F   C P / M   *****************
